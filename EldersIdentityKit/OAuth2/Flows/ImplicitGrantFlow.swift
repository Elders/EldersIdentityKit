...
open class ImplicitGrantFlow: AuthorizationGrantFlow {
    ...
    open func authorizationResponse(from request: URLRequest) throws -> AuthorizationResponse {
        ...
        let parameters = url.fragment?.urlDecodedParameters
            ...
            throw EldersIdentityKitError.authenticationFailed(reason: EldersIdentityKitError.Reason.invalidAccessTokenResponse)
        }
    ...
    open func validate(_ authorizationResponse: AuthorizationResponse) throws {
        ...
        guard authorizationResponse.state == self.state else {
            ...
            throw EldersIdentityKitError.authenticationFailed(reason: EldersIdentityKitError.Reason.invalidAuthorizationResponse)
        }
    }
    ...
    public func authenticate(handler: @escaping @Sendable @MainActor(AccessTokenResponse?, Error?) -> Void) {
        ...
        self.perform(authorizationURLRequest, redirectURI: self.redirectURI) { [weak self] (redirectRequest) throws -> Bool in
            ...
            guard let _self = self else {
                ...
                throw fail(with: EldersIdentityKitError.authenticationFailed(reason: EldersIdentityKitError.Reason.general(message: "Flow was deallocated")))
            }
